...
- Another core-ops skill or agent reports that `<config-root>/plugins/core-ops.user-context.md` is missing — auto-route here

## Quick path

If the user just wants to skip the interview and use defaults: write a minimal `<config-root>/plugins/core-ops.user-context.md` with placeholders, note that `/pipeline-analyst` and `/review-deliverable` will work in degraded mode (default scoring weights, generic brand checks), and tell them to re-run `/setup-core` whenever they're ready.
